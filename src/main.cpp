...
// Copyright (c) 2013 Thunder developers
...
const string strMessageMagic = "Thunder Signed Message:\n";
...
static const int64 nTargetTimespan = 300; // 1 minutes
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ThunderMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ThunderMiner : ProcessBlock, block not accepted");
    }
...
void static BitcoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running ThunderMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ThunderMiner terminated\n");
        throw;
    }
}
